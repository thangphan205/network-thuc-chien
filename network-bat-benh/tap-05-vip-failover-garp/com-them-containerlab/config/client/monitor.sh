...
echo "--------------------------------------------------------------------------------"
printf "%-10s | %-12s | %-22s | %-10s | %-25s\n" "TIME" "STATUS" "BACKEND" "LATENCY" "ARP GATEWAY"
echo "--------------------------------------------------------------------------------"

while true; do
    TIMESTAMP=$(date +"%H:%M:%S")
    GW_MAC=$(ip neigh show 172.28.51.254 2>/dev/null | awk '{print $5}')
    [ -z "$GW_MAC" ] && GW_MAC="(none)"
    ...
    if [ "${CURL_ERR:-0}" -eq 0 ]; then
        ...
        printf "\033[0;32m%-10s | %-12s | %-22s | %-9ss | GW MAC: %s\033[0m\n" \
        ...
        printf "\033[0;31m%-10s | %-12s | %-22s | %-10s | GW MAC: %s\033[0m\n" \
