...
NETWORK_DEVICES = [
    {"hostname": "cisco", "ip_address": "192.168.11.11", "type": "cisco_ios"},
    {"hostname": "arista", "ip_address": "192.168.11.12", "type": "arista_ceos"},
    {"hostname": "juniper", "ip_address": "192.168.11.13", "type": "juniper_junos"},
]
...
def backup_device_config(device, username, password):
    ...
    try:
        ...
        elif device["type"] == "arista_ceos":
            # Disable pagination (for Arista EOS)
            shell.send("enable\n")
            shell.send("terminal length 0\n")
            shell.send("show running-config\n")
            time.sleep(5)  # Wait for the command to execute and the output to be sent
        elif device["type"] == "juniper_junos":
            # Disable pagination (for Juniper Junos)
        ...
        backup_filename = f"ssh-paramiko-v1.1_{device["hostname"]}.cfg"
        ...
        print(
            f"Authentication failed for {device["hostname"]}. Please check credentials."
        )
    except paramiko.SSHException as ssh_e:
        print(f"SSH error on {device["hostname"]}: {ssh_e}")
    except Exception as e:
        print(f"An unexpected error occurred for {device["hostname"]}: {e}")
