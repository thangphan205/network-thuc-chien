...
| 🔄 **CI/CD cho Network Config** | [`cicd-network-config/`](./cicd-%20network-config) | [Kênh Network Thực Chiến](https://www.youtube.com/@NetworkThucChien) | So sánh 4 mô hình NetDevOps: GitOps Push, GitOps Pull, Validation/Canary-Rollback, Golden-config Drift-detection |
...
| 🦇 **Batfish Analysis** | [`batfish/`](./batfish) | [Kênh Network Thực Chiến](https://www.youtube.com/@NetworkThucChien) | Phân tích và kiểm thử tự động cấu hình mạng (Control/Data plane), phát hiện lỗi ACL/BGP và so sánh tác động trước khi deploy (CI/CD) |
