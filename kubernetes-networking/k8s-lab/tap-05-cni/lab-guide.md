...
## 🚀 Thí nghiệm 1: Chuẩn bị "Đồ nghề" (cnitool)
...
## 📜 Thí nghiệm 2: Soạn thảo Hợp đồng CNI (.conflist)
...
## ⚡ Thí nghiệm 3: Đóng vai Kubelet gọi "Lệnh Bài" ADD
...
3. Kiểm chứng xem CNI có nói dối không bằng cách chui vào Namespace đó xem IP:
   ...
   ```
   *Kết quả:* Thật vi diệu! Card `eth0` đã xuất hiện với IP `10.99.0.10` và có sẵn bảng định tuyến chỉa ra Gateway `10.99.0.1` đúng y như hợp đồng! Nếu bạn gõ `ip link show mybridge0` ngoài Node, bạn cũng sẽ thấy switch ảo vừa được tạo.

---

## 🗑 Thí nghiệm 4: Thu hồi mạng bằng lệnh DEL
...
2. Kiểm tra lại Namespace:
   ...
   sudo ip netns exec cni-test ip addr
   ```
   *Kết quả:* Card mạng `eth0` đã biến mất không để lại dấu vết. Địa chỉ IP `10.99.0.10` đã được trả về kho (IPAM) để sẵn sàng cấp cho Pod khác.
