...
# ==========================================
# Script Setup Lab K8s 3 Nodes - Kiến trúc AMD/Intel
# Tối ưu hóa cho chip AMD Ryzen, Intel Core (Windows, Linux, macOS Intel)
...
echo "============================================================"
echo "🚀 KHỞI TẠO MÔI TRƯỜNG K8S LAB - KIẾN TRÚC AMD/INTEL (AMD64) 🚀"
echo "============================================================"
...
if [[ "$ARCH" == "arm64" || "$ARCH" == "aarch64" ]]; then
    ...
    echo "⚠️  Máy của bạn đang chạy chip: $ARCH (Apple Silicon / ARM)."
    echo "👉 Hãy sử dụng script dành riêng cho ARM: ./setup-lab-arm.sh"
    exit 1
fi

echo "✅ Xác thực kiến trúc phần cứng thành công: $ARCH (x86_64/AMD64)"
...
then
    if [[ "$OSTYPE" == "darwin"* ]]; then
        ...
        echo "Multipass chưa được cài đặt. Đang tiến hành cài đặt qua Snap..."
...
# 3. Gợi ý Driver ảo hóa dựa trên Hệ điều hành
if [[ "$OSTYPE" == "darwin"* ]]; then
    echo "🍎 Phát hiện hệ điều hành macOS trên chip Intel/AMD."
    CURRENT_DRIVER=$(multipass get local.driver 2>/dev/null || echo "unknown")
    echo "ℹ️  Driver ảo hóa hiện tại: $CURRENT_DRIVER"
    # Mac Intel trước đây thường dùng hyperkit hoặc virtualbox
    if [[ "$CURRENT_DRIVER" == "qemu" ]]; then
        echo "💡 Gợi ý: Bạn có thể sử dụng driver 'qemu' mặc định hoặc 'hyperkit' nếu gặp vấn đề hiệu năng."
    fi
elif [[ "$OSTYPE" == "msys"* || "$OSTYPE" == "cygwin"* ]]; then
    echo "💻 Phát hiện hệ điều hành Windows."
    echo "💡 Gợi ý: Đảm bảo bạn đã kích hoạt tính năng Hyper-V của Windows để Multipass hoạt động với hiệu năng tốt nhất!"
fi

echo "🚀 Bắt đầu tạo 3 máy ảo Ubuntu 26.04 (AMD64) bằng Multipass..."
...
echo "============================================================"
echo "🎉 HOÀN THÀNH SETUP LAB MÁY ẢO CHO CHIP AMD/INTEL!"
echo "============================================================"
multipass list
