...
| **Lab 3 (Tập 20): Cross-Namespace** | Prometheus ở namespace `monitoring` không scrape được Backend ở `production`. | Lỗi cú pháp dấu gạch ngang tạo logic OR thay vì AND, đồng thời bị che giấu bởi namespace thiếu nhãn (Bug Masking). | `kubectl get ns --show-labels`<br>Phân tích cú pháp YAML `- namespaceSelector` | Sử dụng cú pháp AND (gộp chung dưới 1 dấu gạch ngang), gắn nhãn namespace hoặc dùng K8s auto-labeling. |
| **Lab 4 (Tập 21): Network Policy Nâng Cao** | Tenant A reach được Tenant B (cross-namespace). Backend gọi ra Internet thoải mái. Mọi Pod đều reach được IP quản trị / Metadata `169.254.169.254`. | Chưa có NetworkPolicy nào — cluster mặc định allow-all. Thiếu egress control, thiếu cluster-wide baseline security. | `kubectl get networkpolicy -A`<br>`calicoctl get globalnetworkpolicy`<br>`kubectl exec -- nc -zv <IP> <port>` | Default-deny ingress theo namespace + DNS whitelist (port 53) + `NetworkSet` CIDR egress + `GlobalNetworkPolicy order:1` block IP quản trị/metadata toàn cụm. |
