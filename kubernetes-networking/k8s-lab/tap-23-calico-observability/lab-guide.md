...
3. Kiểm tra xem cấu hình ServiceMonitor đã được Prometheus load thành công chưa:
   ```bash
   # Thiết lập port-forward sang Prometheus service (lắng nghe trên 0.0.0.0 để laptop truy cập được)
...
Ta sẽ thực hiện giả lập kịch bản ứng phó sự cố bảo mật (Security Incident Response) theo đúng quy trình 6 bước chuẩn doanh nghiệp:

```mermaid
flowchart TD
    P1["Bước 1: Giám sát Baseline\n(Lưu lượng hợp lệ & Quét tự do)"] --> P2
    P2["Bước 2: Kích hoạt Lớp Phòng Thủ\n(Áp dụng NetworkPolicy cô lập DB)"] --> P3
    P3["Bước 3: Phát hiện Xâm Nhập\n(Attacker bị chặn & Packet Drop tăng)"] --> P4
    P4["Bước 4: Cảnh báo Tự động\n(Prometheus Rule -> Telegram Alarm)"] --> P5
    P5["Bước 5: Xử lý & Khắc phục\n(Điều tra, cô lập/xoá Attacker Pod)"] --> P6
    P6["Bước 6: Xác nhận Phục hồi\n(Hệ thống tự vá, Telegram Resolved)"]
...
1. Kiểm tra lại kết nối từ **Attacker Pod**:
   ...
   ```
   *Nhận xét:* Gói tin quét của Attacker đã bị Calico Felix chặn đứng hoàn toàn (Silent Drop).

2. **Theo dõi Telemetry (Chỉ số mạng):** Do Attacker liên tục thực hiện vòng lặp quét cổng (mỗi 0.2s), số lượng packet bị drop sẽ tích lũy liên tục. Hãy truy vấn thẳng **Counter thô** trên Prometheus:
   ```bash
   # Truy cập qua NodePort của Prometheus trên máy controlplane
...
1. Theo dõi tốc độ packet drop tăng vọt (pkt/s) thời gian thực trên Node:
   ```bash
...
### Bước 5: Xử lý & Khắc phục Sự cố

Dựa vào thông tin cảnh báo (Node bị ảnh hưởng, loại lỗi), đội ngũ DevSecOps tiến hành điều tra, phát hiện `attacker-pod` đang gửi lưu lượng quét trái phép và thực hiện **cô lập bằng cách xóa pod tấn công**:

```bash
kubectl delete pod attacker-pod -n default
```

### Bước 6: Xác nhận Phục hồi (Auto-Recovery)

Sau khi xóa Pod tấn công, lưu lượng quét trái phép chấm dứt:
1. Chỉ số `felix_denied_packets_total` ngừng tăng và tốc độ drop rate giảm về `0`.
2. Theo cấu hình `resolve_timeout: 5m` trong Helm values, Alertmanager sẽ theo dõi cụm trong vòng 5 phút. Nếu hoàn toàn im lặng và không phát sinh thêm packet drop, hệ thống sẽ tự động gửi tin nhắn phục hồi về Telegram:
...
## Thực nghiệm 9: Tích hợp GitOps Dashboard & Giám sát Trực quan trên Grafana
...
3. Gán nhãn (label) để tính năng Grafana Sidecar tự động nhận diện và nạp Dashboard vào giao diện:
   ...
   ```
   *Giải thích:* Bộ lọc sidecar của Helm chart sẽ quét các ConfigMap có nhãn `grafana_dashboard="1"` và tự động import vào thư mục Dashboards của Grafana mà không cần restart pod.

### Bước 2: Truy cập Grafana từ Trình duyệt Laptop

Do dịch vụ Grafana đã được phơi ra ngoài vĩnh viễn qua cổng NodePort **`32300`**, bạn không cần dùng lệnh `kubectl port-forward` nữa.
