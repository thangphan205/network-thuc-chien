# Lab Tập 30: L7 Policy — Chặn HTTP POST theo path với Envoy Proxy
...
## 🛠 Yêu cầu chuẩn bị
- Cilium đang chạy với Hubble enabled (từ Tập 24).
...
2. Baseline test — tất cả paths accessible trước khi có policy:
   ...
   # 200 ← Chưa có L7 policy, không bị block
   ```
...
3. Test POST /api/data — ALLOWED:
   ```bash
   kubectl -n production exec frontend -- \
     curl -s -o /dev/null -w "%{http_code}" \
     -X POST http://$BACKEND_IP:8080/api/data -d '{}'
   # 200 ✅ POST /api/* allowed
   ```
...
1. Verify Envoy listener được tạo khi có L7 policy:
   ...
   # 1234      Enabled           Disabled          ← L7 policy active!
   ```
...
2. Xem Envoy listener config (nếu có cilium CLI support):
   ...
   # ← Envoy đang handle L7 redirect cho 1 endpoint
   ```
...
3. Generate mixed traffic:
   ...
   # production/frontend → production/backend  HTTP POST /api/users 200 FORWARDED
   # production/frontend → production/backend  HTTP POST /admin/users 403 DROPPED
   ```
