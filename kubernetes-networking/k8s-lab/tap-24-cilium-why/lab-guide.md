# Lab Tập 24: Dựng cụm Cilium Production — kube-proxy Replacement + Native Routing + WireGuard + Hubble
...
```bash
# Trên host machine — chạy từ thư mục tap-24-cilium-why/
# Mac Apple Silicon:
...
Cilium CLI (`cilium`) cung cấp lệnh `cilium status`, `cilium connectivity test`, `cilium bpf *` — dùng xuyên suốt từ Tập 24 đến 40.
...
**Giữ lại cluster** — dùng cho toàn bộ Tập 25–40.
...
5. **Hubble built-in = zero-setup observability:** `hubble observe --verdict DROPPED` cho ngay flow nào bị deny và tại sao — không cần tcpdump, không cần log parser. Dùng xuyên suốt Tập 33–40.
