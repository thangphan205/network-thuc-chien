...
sequenceDiagram
    ...
    participant Felix as Felix (mỗi Node)
    participant Kernel as Linux Kernel
    participant BIRD as BIRD (mỗi Node)
    participant Peers as Các Node khác

    rect rgb(219, 234, 254)
        Note over Admin,Kernel: Luồng 1 — Đồng bộ Network Policy
        Admin->>API: kubectl apply NetworkPolicy
        API-->>Typha: Stream sự kiện thay đổi
        Note over Typha: Cache giúp 500 Felix<br/>không kết nối thẳng API Server
        Typha-->>Felix: Đẩy config xuống mọi Node
        Felix->>Kernel: Ghi iptables / eBPF rules (< 100ms)
    end

    rect rgb(209, 250, 229)
        Note over BIRD,Peers: Luồng 2 — Định tuyến BGP
        BIRD->>Kernel: Đọc Pod IP vừa được gán
        BIRD->>Peers: Quảng bá Pod Subnet qua BGP (port 179)
        Peers->>Kernel: Ghi route chéo Node vào Routing Table
    end
```
