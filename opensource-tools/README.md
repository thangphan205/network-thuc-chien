...
## 🔍 Series: Debug Mạng từ A-Z

Bộ tài liệu hệ thống hóa kỹ năng chẩn đoán mạng — từ kiểm tra kết nối cơ bản đến bắt gói tin và benchmark hiệu năng.

> **[📂 Xem lộ trình học & kịch bản thực chiến tổng hợp](./debug-mang-az)**
...
| **ping** | Kiểm thử & Xử lý sự cố | ICMP connectivity test — MTU discovery, TTL fingerprinting, flood ping. | [📂 `./ping`](./ping) |
| **MTR** | Kiểm thử & Xử lý sự cố | Khám bệnh mạng toàn diện. Sự kết hợp hoàn hảo giữa Ping và Traceroute. | [📂 `./mtr`](./mtr) |
| **ss** | Kiểm thử & Xử lý sự cố | X-quang socket — thay thế netstat, đọc TCP states, phát hiện port leak. | [📂 `./ss`](./ss) |
| **netcat (nc)** | Kiểm thử & Xử lý sự cố | Dao Thụy Sĩ TCP/UDP — test port, tạo server tạm, debug firewall rule. | [📂 `./netcat`](./netcat) |
| **dig** | Kiểm thử & Xử lý sự cố | DNS debug chuyên nghiệp — trace delegation, kiểm tra TTL, debug K8s CoreDNS. | [📂 `./dig`](./dig) |
| **curl** | Kiểm thử & Xử lý sự cố | HTTP debug từ terminal — timing breakdown, TLS cert, redirect chain, auth. | [📂 `./curl`](./curl) |
| **tcpdump** | Kiểm thử & Xử lý sự cố | Bắt gói tin dòng lệnh — filter syntax, TCP flags, remote capture cho Wireshark. | [📂 `./tcpdump`](./tcpdump) |
| **iPerf3** | Kiểm thử & Xử lý sự cố | Benchmark băng thông chuẩn mực — TCP/UDP throughput, jitter, parallel streams. | [📂 `./iperf3`](./iperf3) |
