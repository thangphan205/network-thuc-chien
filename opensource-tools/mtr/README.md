...
Đôi khi việc phân giải Reverse DNS cho từng bước nhảy (hop) mất rất nhiều thời gian và làm công cụ hiển thị chậm chạp. Hãy dùng cờ `-n` để hiển thị trực tiếp IP tĩnh.
```bash
mtr -n google.com
```
...
### 5. Ép xung (Thay đổi tần suất gửi)
...
Độ lệch chuẩn (**StDev**) trong MTR chính là chỉ số dùng để đo lường **Jitter** (sự dao động/chập chờn của độ trễ mạng).
